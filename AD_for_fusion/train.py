...
from models.model import CombinedGAT
...
for epoch in range(config['train']['epochs']):
    ...
    correct = 0
    total = 0
    for high_dim_features, low_dim_features, labels, edge_index in train_loader:
    ...
    with torch.no_grad():
        for high_dim_features, low_dim_features, labels, edge_index in val_loader:  # 假设你有一个验证集加载器val_loader
            high_dim_features = high_dim_features.to(device)
            low_dim_features = low_dim_features.to(device)
            labels = labels.to(device)
            edge_index = edge_index.to(device)
