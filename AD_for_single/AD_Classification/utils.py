...
def calculate_correlation_per_sample(feature_matrix, mean, var):
    ...
    for i in range(N):
        ...
        # 将相关系数的绝对值转换为0到1的范围
        correlation_matrix = torch.abs(correlation_matrix) / torch.max(correlation_matrix, dim=1, keepdim=True)[0]
...
def cov_builder_PET(high_dim_features,label):
    labels_CN = label == 0
    ...
    correlation_matrices_c1[torch.isnan(correlation_matrices_c1)] = 0
    
    return correlation_matrices_c1.unsqueeze(1)
...
def individual_feature_builder_PET(high_dim_features,label):
    labels_CN = label == 0
    ...
    flatten_matrix_c1[torch.isnan(flatten_matrix_c1)] = 0
    return flatten_matrix_c1
